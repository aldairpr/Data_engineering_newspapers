...
def main(filename):
    ...
    df = _add_newspaper_uid_coulmn(df, newspaper_uid)
    df = _extract_host(df)
    df = _fill_missing_titles(df)
    df = _generate_uids_for_rows(df)
    df = _remove_new_lines_from(df, 'title')
    df = _remove_new_lines_from(df, 'body')
...
def _extract_newspaper_uid(filename):
    logger.info('Estract newspaper uid')
...
def _add_newspaper_uid_coulmn(df, newspaper_uid):
...
def _remove_new_lines_from(df, column_name):
    logger.info('Remove new lines from {}'.format(column_name))
